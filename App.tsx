...
// Firebase imports
import { db, auth, initError } from './firebaseConfig';
  ...
  return (
    <div className="min-h-screen pb-12 bg-slate-950 text-slate-200">
          ...
          <div className="animate-fadeIn">
            <div className="flex justify-between items-center mb-6">
              <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-800 shadow-inner">
                <button onClick={() => setAdventureSubTab('map')} className={`px-4 py-2 text-xs font-bold rounded transition-all ${adventureSubTab === 'map' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>🗺️ 夥伴大冒險</button>
                <button onClick={() => setAdventureSubTab('dispatch')} className={`px-4 py-2 text-xs font-bold rounded transition-all ${adventureSubTab === 'dispatch' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>🕒 派遣工作</button>
              </div>
              <div className="flex gap-2">
                {adventureSubTab === 'dispatch' && <button onClick={() => setIsDispatchGuideOpen(true)} className="px-3 py-2 bg-blue-900/40 text-blue-300 text-xs font-bold rounded border border-blue-700/50 hover:bg-blue-800 transition shadow">派遣指南</button>}
      ...
      <div className="relative z-[60]">
        ...
        {/* Fish & Item Modals */}
        {isFormModalOpen && <FishFormModal initialData={editingFish} existingIds={fishList.map(f=>f.id)} onSave={async (f) => {if(db) await setDoc(doc(db, "fishes", f.id), f); setIsFormModalOpen(false);}} onClose={() => setIsFormModalOpen(false)} />}
        {selectedDetailFish && <FishDetailModal fish={selectedDetailFish} onClose={() => setSelectedDetailFish(null)} />}
        {isItemFormModalOpen && <ItemFormModal initialData={editingItem} onSave={async (i) => {if(db) await setDoc(doc(db, "items", i.id || Date.now().toString()), i); setIsItemFormModalOpen(false);}} onClose={() => setIsItemFormModalOpen(false)} itemList={itemList} />}
