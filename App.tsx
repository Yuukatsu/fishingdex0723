...
import BattleFormSkillCard from './components/BattleFormSkillCard';
import BattleFormSkillFormModal from './components/BattleFormSkillFormModal';
import BattleFormSkillDetailModal from './components/BattleFormSkillDetailModal';
...
const App: React.FC = () => {
  ...
  const [battleFormSkills, setBattleFormSkills] = useState<BattleFormSkill[]>([]);
  ...
  useEffect(() => {
      if (!db) return;
      const q = query(collection(db, "battle_form_skills"));
      const unsubscribe = onSnapshot(q, (snapshot) => {
          const fetchedSkills: BattleFormSkill[] = [];
          snapshot.forEach((doc) => {
              const data = doc.data() as BattleFormSkill;
              fetchedSkills.push(data);
          });
          fetchedSkills.sort((a, b) => {
              const numA = a.cardNumber ?? 999999;
              const numB = b.cardNumber ?? 999999;
          ...
          setBattleFormSkills(fetchedSkills);
      });
      return () => unsubscribe();
  }, []);
  ...
  const filteredBattleFormSkills = useMemo(() => {
      ...
      if (specialSkillSearchTerm) {
          ...
          filtered = filtered.filter(f => 
              ...
              (f.cardNumber && f.cardNumber.toString().includes(lower)) ||
              (f.partner.note && f.partner.note.toLowerCase().includes(lower))
          );
      }
      return filtered;
  ...
  const handleSaveSocialLinks = async (links: SocialLinks) => {
      if (!db || !currentUser) return alert("權限不足：請先登入");
      try {
          await setDoc(doc(db, "app_settings", "social_links"), links);
  ...
  const handleSaveMainSkill = async (skill: MainSkill) => { if (!db || !currentUser) return alert("權限不足"); try { const id = skill.id || Date.now().toString(); const dataToSave = { ...skill, id }; Object.keys(dataToSave).forEach(key => { if ((dataToSave as any)[key] === undefined) delete (dataToSave as any)[key]; }); await setDoc(doc(db, "main_skills", id), dataToSave); setIsMainSkillFormOpen(false); setEditingMainSkill(null); } catch (e: any) { alert(`儲存失敗: ${e.message}`); } };
  ...
  const handleSaveSubSkill = async (skill: SubSkill) => { if (!db || !currentUser) return alert("權限不足"); try { const id = skill.id || Date.now().toString(); const dataToSave = { ...skill, id }; Object.keys(dataToSave).forEach(key => { if ((dataToSave as any)[key] === undefined) delete (dataToSave as any)[key]; }); await setDoc(doc(db, "sub_skills", id), dataToSave); setIsSubSkillFormOpen(false); setEditingSubSkill(null); } catch (e: any) { alert(`儲存失敗: ${e.message}`); } };
  ...
  const handleSaveGuide = async (guide: SystemGuide) => { if (!db || !currentUser) return alert("權限不足"); try { const id = guide.id || Date.now().toString(); const dataToSave = { ...guide, id }; Object.keys(dataToSave).forEach(key => { if ((dataToSave as any)[key] === undefined) delete (dataToSave as any)[key]; }); await setDoc(doc(db, "system_guides", id), dataToSave); setIsGuideFormOpen(false); setEditingGuide(null); } catch (e: any) { alert(`儲存失敗: ${e.message}`); } };
  ...
  return (
    <div className="min-h-screen pb-12 transition-colors duration-500 bg-slate-950">
          ...
          <div className="flex flex-col gap-4 py-4">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 self-end md:self-center">
                   {isDevMode && (
                       <button onClick={() => setIsSocialLinksModalOpen(true)} className="p-1.5 text-slate-500 hover:text-white transition-colors" title="編輯社群連結">
                           ⚙️
                       </button>
                   )}
                   {socialLinks.discord && (
                       <a href={socialLinks.discord} target="_blank" rel="noopener noreferrer" className="p-1.5 text-slate-400 hover:text-[#5865F2] transition-colors" title="Discord">
                   ...
                   {socialLinks.youtube && (
                       <a href={socialLinks.youtube} target="_blank" rel="noopener noreferrer" className="p-1.5 text-slate-400 hover:text-[#FF0000] transition-colors" title="YouTube">
                           <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                       </a>
                   )}
                   {socialLinks.twitch && (
                       <a href={socialLinks.twitch} target="_blank" rel="noopener noreferrer" className="p-1.5 text-slate-400 hover:text-[#9146FF] transition-colors" title="Twitch">
      ...
      {isBattleFormSkillFormOpen && <BattleFormSkillFormModal initialData={editingBattleFormSkill} onSave={handleSaveBattleFormSkill} onClose={() => setIsBattleFormSkillFormOpen(false)} />}
      {selectedDetailBattleFormSkill && <BattleFormSkillDetailModal skill={selectedDetailBattleFormSkill} onClose={() => setSelectedDetailBattleFormSkill(null)} />}
      ...
      {selectedDetailItem && <ItemDetailModal item={selectedDetailItem} onClose={() => { setSelectedDetailItem(null); setSelectedDetailItemTab('normal'); }} isDevMode={isDevMode} itemList={itemList} initialTab={selectedDetailItemTab} onShowSource={(item) => setSelectedSourceItem(item)} mapList={mapList} encounterList={encounterList} fishList={fishList} dispatchList={dispatchList} />}
      {selectedSourceItem && (
        <ItemSourceModal
