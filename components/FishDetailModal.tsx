    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn" onClick={onClose}>
      ...
      >
        ...
        <div className="w-full h-[45vh] min-h-[300px] bg-slate-950 relative border-b border-slate-800 flex flex-col items-center justify-center flex-shrink-0">
          ...
          {/* Variant Controls Overlay in Image Area */}
          <div className="absolute bottom-4 left-4 flex gap-2 bg-black/50 p-1.5 rounded-lg backdrop-blur-sm">
             <button onClick={() => setCurrentVariant('normalMale')} className={`px-3 py-1 text-xs rounded font-bold transition ${currentVariant === 'normalMale' ? 'bg-blue-600 text-white shadow' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}>一般♂</button>
             <button onClick={() => setCurrentVariant('normalFemale')} className={`px-3 py-1 text-xs rounded font-bold transition ${currentVariant === 'normalFemale' ? 'bg-pink-600 text-white shadow' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}>一般♀</button>
             <button onClick={() => setCurrentVariant('shinyMale')} className={`px-3 py-1 text-xs rounded font-bold transition ${currentVariant === 'shinyMale' ? 'bg-amber-600 text-white shadow' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}>異色♂</button>
             <button onClick={() => setCurrentVariant('shinyFemale')} className={`px-3 py-1 text-xs rounded font-bold transition ${currentVariant === 'shinyFemale' ? 'bg-purple-600 text-white shadow' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}>異色♀</button>
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 gap-y-4 text-sm">
             <div className="flex border-b border-slate-800 pb-2">
                ...
                <span className="flex-1 text-slate-200 text-base">
                  {depthDisplay}
                </span>
             </div>
